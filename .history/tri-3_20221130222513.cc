...
#define X_BORDER (((float)X_PIX + 0.5f) / (float)WIN_X) * 2.0f - 1.0f
#define Y_BORDER = 1.0f - (((float)Y_PIX + 0.5f) / (float)WIN_Y) * 2.0f;
...
public:
	...
	void update(){
		for (vec2f vec : mat){
			if (vec.x >= X_BORDER || vec.x <= -X_BORDER)
				x_dir *= -1;

			if (vec.y >= Y_BORDER || vec.y <= -Y_BORDER)
				y_dir *= -1;
		}
	}
