...
struct Triangle {
	...
	void printCoords(){
		printf("(%f,%f)\n(%f,%f)\n(%f,%f)\n", v_pos[0].x, v_pos[0].y, v_pos[1].x, v_pos[1].y, v_pos[2].x, v_pos[2].y);
	}
};
...
int main(int argc, char** argv) {
	...
	while (!glfwWindowShouldClose(window)){
		...
		for (vec2 v_pos : tri.v_pos){
			v_pos.x += x_vel * tri.x_dir;
		}
		for (vec2 v_pos : tri.v_pos){
			if (v_pos.x >= 1 || v_pos.x <= -1)
				tri.x_dir *= -1;
			if (v_pos.y >= 1 || v_pos.y <= -1)
				tri.y_dir *= -1;
		}
		tri.printCoords();
	}
