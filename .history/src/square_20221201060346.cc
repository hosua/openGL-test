...
class Square {
   ...
   public:
    ...
    Square(GLfloat scalar, float x_vel, float y_vel, int x_dir, int y_dir,
           float rot_vel, int rot_dir)
        : x_vel(x_vel),
          y_vel(y_vel),
          x_dir(x_dir),
          y_dir(y_dir),
...
vector<Square> makeSquares(size_t count){
    vector<Square> v;
    while (count--)
...
int main(int argc, char** argv) {
    ...
    for (int i = 0; i < sq_count; i++) {
        Square s = Square(0.25, 0.01, 0.008f, 1, 1, 0.5f, -1);
        sq_vec.push_back(s);
    }
    // Square sq = Square(0.25, 0.01, 0.008f, 1, 1, 0.5f, -1);
    ...
    while (!glfwWindowShouldClose(window)) {
        ...
        glBegin(GL_QUADS);
        for (Square& sq : sq_vec){
            cout << sq_vec.size() << endl;
            sq.draw();
            sq.printPos();
            sq.rotate();
            sq.move();
