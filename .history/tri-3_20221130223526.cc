...
public:
	...
	void update(){
		float max_x = *max_element({x1, x2, x3});
		float max_y = *max_element({y1, y2, y3});
		float min_x = *min_element({x1, x2, x3});
		float min_y = *min_element({y1, y2, y3});
		if (max_x >= 1 || min_x <= -1)
			x_dir *= -1;
		...
		if (min_y >= 1 || min_y <= -1)
			y_dir *= -1;
		cout << "min_y: " << min_y << "\t" << "min_x: " << min_x << endl;
		// for (vec2f vec : mat){
		// 	if (vec.x >= 1 || vec.x <= -1)
		// 		x_dir *= -1;
